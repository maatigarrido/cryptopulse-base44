...
const YEARS = [
  { name: "Año 2012", startDate: new Date("2012-01-01"), endDate: new Date("2016-01-01"), color: "#4e8cff" },
  { name: "Año 2016", startDate: new Date("2016-01-01"), endDate: new Date("2020-01-01"), color: "#f97316" },
  { name: "Año 2020", startDate: new Date("2020-01-01"), endDate: new Date("2024-01-01"), color: "#22c55e" },
  { name: "Año 2024", startDate: new Date("2024-01-01"), endDate: null, color: "#ef4444" },
];

const LOG_TICKS = [0.5, 1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000];
const MARGIN = { top: 10, right: 20, left: 58, bottom: 40 };
const HEIGHT = 420;
const INIT_OFFSETS = () => Object.fromEntries(YEARS.map((y) => [y.name, { dx: 0, dy: 1 }]));

const formatUSD = (v) =>
  v != null ? `$${v.toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : null;

export default function YearChart({ dailyData }) {
  ...
  // For each year cycle, map day offset → real price
  const priceByYearDay = useMemo(() => {
    const result = {};
    YEARS.forEach(({ name, startDate }) => {
  ...
  const rawByYear = useMemo(() => {
    ...
    const map = {};
    YEARS.forEach(({ name, startDate, endDate }) => {
      const startTs = startDate.getTime() / 1000;
      const endTs = endDate ? endDate.getTime() / 1000 : Infinity;
      const slice = dailyData.filter((d) => d.x >= startTs && d.x < endTs);
      if (!slice.length) return;
      const basePrice = slice[0].y;
      map[name] = slice.map((d) => ({
  ...
  const baseMaxDay = useMemo(() => {
    let max = 1460;
    Object.values(rawByYear).forEach((arr) => {
      if (arr.length) max = Math.max(max, arr[arr.length - 1].day);
    });
    return max;
  }, [rawByYear]);
  ...
  const domainX = useMemo(() => {
    let min = 0, max = baseMaxDay;
    Object.values(shiftedSeries).forEach((s) => s.forEach(({ day }) => {
      if (day < min) min = day;
      if (day > max) max = day;
    }));
    return [min, max];
  }, [shiftedSeries, baseMaxDay]);

  const domainY = useMemo(() => {
    let maxVal = 200;
    Object.values(rawByYear).forEach((series) => {
  ...
  const getValuesAtDay = useCallback((day) => {
    return YEARS.map(({ name, color }) => {
      ...
      const price = priceByYearDay[name]?.(origDay);
      const yearStart = YEARS.find((y) => y.name === name)?.startDate;
      const date = yearStart ? new Date((yearStart.getTime() / 1000 + origDay * 86400) * 1000) : null;
      return { name, color, value: best.value, day: best.day, price, date };
    }).filter(Boolean);
  }, [shiftedSeries, priceByYearDay]);
  ...
  }, [shiftedSeries, W, editMode, domainX, domainY, offsets, dailyData, selectedLine, tooltip, xToPixel, yToPixel]);
    ...
    <div>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <p className="text-xs text-muted-foreground">
          Multiplicador del Precio desde el 1 de Enero (Escala Logarítmica)
        </p>
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-semibold transition-all ${editMode ? "bg-primary text-primary-foreground shadow" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
          >
